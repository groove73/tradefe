...
import { fetchBeneficiaryQuotation, FscBeneficiaryCertificate } from '@/lib/stock-quotation';
import { BeneficiaryCertificateTable } from '@/components/stocks/BeneficiaryCertificateTable';
...
function BeneficiaryCertificateContent() {
    ...
    const [data, setData] = useState<FscBeneficiaryCertificate[]>([]);
    ...
    const loadData = useCallback(async (targetPage: number, targetDate: string, search: string, match: string) => {
        ...
        try {
            ...
            const result = await fetchBeneficiaryQuotation(targetDate, targetPage, itemsPerPage, itmsNm, likeItmsNm);
            setData(result.items);
            setTotalCount(result.totalCount);
        } catch (error) {
            console.error('Failed to load beneficiary certificate data:', error);
        } finally {
            setLoading(false);
        }
    }, []);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                <div>
                    <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight flex items-center gap-3">
                        <span className="text-emerald-600 uppercase italic">수익증권</span> Dashboard
                    </h1>
                    <p className="text-gray-500 mt-2 font-medium italic">KRX 에 상장된 수익증권의 시세 정보 상세</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <Button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl px-6 h-10 shadow-md shadow-emerald-100 transition-all hover:scale-105"
                    >
...
export default function BeneficiaryCertificatePage() {
    return (
        <Suspense fallback={<div className="container mx-auto py-12 px-4 max-w-6xl animate-pulse text-gray-400 font-bold">데이터를 불러오는 중...</div>}>
            <BeneficiaryCertificateContent />
        </Suspense>
    );
}

...
import { FscBeneficiaryCertificate } from '@/lib/stock-quotation';

interface BeneficiaryCertificateTableProps {
    data: FscBeneficiaryCertificate[];
    totalCount: number;
    currentPage: number;
    itemsPerPage: number;
...
export function BeneficiaryCertificateTable({
    data,
    totalCount,
    currentPage,
    itemsPerPage,
    onPageChange
}: BeneficiaryCertificateTableProps) {
                ...
                <Table>
                    <TableHeader className="bg-gray-50/50">
                        <TableRow>
                            ...
                            <TableHead className="font-bold text-right">상장좌수</TableHead>
                                ...
                                <TableRow key={`${item.srtCd}-${idx}`} className="hover:bg-gray-50/50 transition-colors">
                                    <TableCell className="font-medium text-gray-900">
                                        <div className="flex flex-col">
                                            <span className="font-bold">{item.itmsNm}</span>
                                    ...
                                    <TableCell className="text-right font-mono text-gray-600">{formatValue(item.lstPnt)}</TableCell>
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                            ...
                            return (
                                <Button
                                    ...
                                    className={`w-10 h-10 rounded-xl font-bold ${currentPage === pageNum ? "bg-emerald-600 text-white shadow-md shadow-emerald-200" : "border-gray-200 text-gray-600"
                                        }`}
                                >
                                    {pageNum}
                                </Button>

...
import { fetchSubscriptionRightQuotation, FscStockSubscriptionRight } from '@/lib/stock-quotation';
import { StockSubscriptionRightTable } from '@/components/stocks/StockSubscriptionRightTable';
...
function SubscriptionRightContent() {
    ...
    const [data, setData] = useState<FscStockSubscriptionRight[]>([]);
    ...
    const loadData = useCallback(async (targetPage: number, targetDate: string, search: string, match: string) => {
        ...
        try {
            ...
            const result = await fetchSubscriptionRightQuotation(targetDate, targetPage, itemsPerPage, itmsNm, likeItmsNm);
            setData(result.items);
            setTotalCount(result.totalCount);
        } catch (error) {
            console.error('Failed to load subscription right data:', error);
        } finally {
            setLoading(false);
        }
    }, []);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                <div>
                    <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight flex items-center gap-3">
                        <span className="text-amber-600 uppercase italic">신주인수권증권</span> Dashboard
                    </h1>
                    <p className="text-gray-500 mt-2 font-medium italic">KRX 에 상장된 신주인수권증권시세 정보 상세</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <Button
                        ...
                        className="bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl px-6 h-10 shadow-md shadow-amber-100 transition-all hover:scale-105"
                    >
...
export default function SubscriptionRightPage() {
    return (
